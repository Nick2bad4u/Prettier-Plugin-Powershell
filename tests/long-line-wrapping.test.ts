...
describe("Long Line Wrapping Improvements", () => {
    describe("Long Pipeline Chains", () => {
        it("breaks long pipelines with 4+ segments", async () => {
            ...
            const result = await formatAndAssert(input, baseConfig, "long-line-wrapping.result");
    ...
    describe("Edge Cases", () => {
        it("handles extremely long single-line scripts (>500 chars)", async () => {
            ...
            const result = await formatAndAssert(longScript, baseConfig, "long-line-wrapping.result");
            expect(result).toBeTruthy();
    ...
    describe("Performance on Long Lines", () => {
        it("formats 1000-character line in reasonable time", async () => {
            ...
            const result = await formatAndAssert(longLine, baseConfig, "long-line-wrapping.result");
        ...
        it("handles deeply nested expressions", async () => {
            ...
            const result = await formatAndAssert(nested, baseConfig, "long-line-wrapping.result");
            expect(result).toBeTruthy();
            expect(result).toContain("$result");
        });
    });
