...
@{
    ...
    # We intentionally use Write-Host for colorscripts
    # Already implemented where needed
    # Manifest files intentionally use standard formatting
    # UTF-8 without BOM is intentional for cross-platform scripts
    # False positives in some scripts
    ...
    Rules =
        @{
            ...
            PSAvoidLongLines = @{ Enable = $false; MaximumLineLength = 1000 }
            ...
            PSAvoidOverwritingBuiltInCmdlets =
                @{
                    Enable = $true
                    PowerShellVersion =
                        @( 'core-6.1.0-windows', 'core-6.1.0-linux' )
                }
            PSReviewUnusedParameter =
                @{ Enable = $true; CommandsToTraverse = @('*') }
            ...
            PSUseSingularNouns = @{ Enable = $true; NounAllowList = @() }
            ...
            PSUseCompatibleSyntax =
                @{ Enable = $true; TargetVersions = @( '5.1', '7.0', '7.2' ) }
        }
