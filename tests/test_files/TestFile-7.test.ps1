...
function Get-ModuleInstallRoot {
  ...
  $paths = $env:PSModulePath -split [System.Text.RegularExpressions.Regex]::Escape(
    $separator
  )
    | Where-Object {
      $_
...
try {
  ...
  if ($PSCmdlet.ShouldProcess( $destinationPath, 'Import module' )) {
    Import-Module (
      Join-Path -Path $destinationPath -ChildPath 'ColorScripts-Enhanced.psd1'
    ) -Force
  ...
  if ($AddToProfile) {
    ...
    if (
      $PSCmdlet.ShouldProcess( $PROFILE.CurrentUserAllHosts, 'Update profile' )
    ) {
  ...
  if ($BuildCache) {
    if (
      $PSCmdlet.ShouldProcess(
        'ColorScripts-Enhanced cache',
        'Build caches for all scripts'
      )
    ) {
      Write-Host ''
      ...
      $successCount = (
        $cacheResults | Where-Object {
          $_.Status -in @( 'Updated', 'SkippedUpToDate' )
        }
      ).Count
      $failureCount = (
        $cacheResults | Where-Object {
