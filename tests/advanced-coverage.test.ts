...
import { parsePowerShell, __parserTestUtils, locStart, locEnd } from '../src/parser.js';
import { __printerTestUtils, createPrinter, powerShellPrinter } from '../src/printer.js';
...
interface ParserTestUtils {
  ...
  parseScriptWithTerminators: (source: string, terminators: Set<string>) => ScriptNode;
...
const containsHardline = (value: unknown): boolean => containsFragment(value, hardline);
...
describe('ast runtime helpers', () => {
  it('createLocation normalizes coordinates and prevents negative spans', () => {
    ...
    expect(createLocation(Number.POSITIVE_INFINITY, 15)).toEqual({ start: 0, end: 15 });
  });
...
describe('resolveOptions advanced coverage', () => {
  ...
  it('clamps blank lines between functions within the allowed range', () => {
    const options = createOptions({ powershellBlankLinesBetweenFunctions: 10 });
    const resolved = resolveOptions(options);
  ...
  it('cascades indent fallbacks and ignores invalid blank-line input', () => {
    ...
    options.powershellBlankLinesBetweenFunctions = Number.NaN as unknown as number;
...
describe('tokenizer advanced coverage', () => {
  ...
  it('tokenizes double colon as a single operator', () => {
    ...
    const operatorToken = tokens.find((token) => token.type === 'operator' && token.value === '::');
...
describe('parser advanced coverage', () => {
  const parse = (source: string, overrides: Partial<PowerShellParserOptions> = {}) =>
  ...
  it('treats newline inside parentheses as part of expression', () => {
    ...
    const parenthesis = pipeline.segments[0]?.parts.find((part) => part.type === 'Parenthesis');
    expect(parenthesis).toBeDefined();
    expect((parenthesis as { hasNewline: boolean } | undefined)?.hasNewline).toBe(true);
  });
  ...
  it('breaks pipeline when newline comment prevents continuation', () => {
    ...
    if (!firstPart || firstPart.type !== 'Text') {
      throw new Error('Expected text part starting the continuation pipeline');
    }
    expect(firstPart.value).toBe('Where-Object');
    expect(segmentParts.some((part) => part.type === 'Text' && part.value === '|')).toBe(false);
  });

  it('splits statements on semicolons and collects blank lines', () => {
    const script = 'Write-Host "One"; Write-Host "Two"\n\n\nWrite-Host "Three"';
    const ast = parse(script);
    const pipelines = ast.body.filter((node) => node.type === 'Pipeline');
  ...
  it('parses complex hashtable entries and nested structures', () => {
    const script = '@{ "quoted" = 1; flag; single = @{ nested = @(1, 2) } }';
    const ast = parse(script);
    const pipeline = ast.body[0];
    if (!pipeline || pipeline.type !== 'Pipeline') {
      throw new Error('Expected pipeline node');
  ...
  it('stops function header scanning when encountering inline comments', () => {
    const script = 'function HeaderComment # comment here\n{ Write-Host "inside" }';
    const ast = parse(script);
    const fn = ast.body.find((node) => node.type === 'FunctionDeclaration');
    if (!fn || fn.type !== 'FunctionDeclaration') {
  ...
  it('handles unterminated hashtables by capturing available tokens', () => {
    ...
    const table = pipeline.segments[0]?.parts.find((part) => part.type === 'Hashtable');
    expect(table).toBeDefined();
    expect((table as { entries: unknown[] } | undefined)?.entries.length).toBe(1);
  });
...
describe('printer advanced coverage', () => {
  it('formats multiple functions with custom spacing options', async () => {
    ...
    expect(blankLineSegments.filter((line) => line.trim() === '').length).toBeGreaterThanOrEqual(3);
  });
  ...
  it('prints pipelines with multiple segments and trailing comments', async () => {
    const script = 'Get-Process | Where-Object { $_.Name } | Select-Object Name # in pipeline';
...
describe('printer internal helpers', () => {
  ...
  it('normalizeStringLiteral respects quoting preferences', () => {
    const preferSingle = resolveOptions(createOptions({ powershellPreferSingleQuote: true }));
    expect(normalizeStringLiteral('"hello"', preferSingle)).toBe("'hello'");
    expect(normalizeStringLiteral('"he"llo"', preferSingle)).toBe('"he"llo"');
    expect(normalizeStringLiteral("'already'", preferSingle)).toBe("'already'");
    expect(normalizeStringLiteral('"needs $expansion"', preferSingle)).toBe('"needs $expansion"');
    const noPreference = resolveOptions(createOptions({ powershellPreferSingleQuote: false }));
    expect(normalizeStringLiteral('"hello"', noPreference)).toBe('"hello"');
  });

  it('printParamParenthesis handles empty element lists', () => {
    const doc = printParamParenthesis(makeParenthesisNode([]), resolvedOptions);
    expect(doc).toBe('()');
  });
  ...
  it('trailingCommaDoc respects trailing comma settings', () => {
    const allComma = resolveOptions(createOptions({ powershellTrailingComma: 'all' }));
    ...
    const multiComma = resolveOptions(createOptions({ powershellTrailingComma: 'multiline' }));
  ...
  it('printScript returns empty doc when body is empty', () => {
    const scriptNode: ScriptNode = { type: 'Script', body: [], loc: { start: 0, end: 0 } };
    expect(__printerTestUtils.printScript(scriptNode, resolvedOptions)).toBe('');
  });
  ...
  it('printNode falls back to empty doc for unknown nodes', () => {
    const unknownNode = { type: 'Unknown', loc: { start: 0, end: 0 } } as unknown as ScriptBodyNode;
  ...
  it('printText preserves keywords for unknown case transforms', () => {
    ...
    expect(__printerTestUtils.printNode(keywordNode, mutatedOptions)).toBe('While');
  });

  it('printText applies pascal keyword transformations', () => {
    const pascalOptions = resolveOptions(createOptions({ powershellKeywordCase: 'pascal' }));
    const keywordNode = makeTextNode('function', 'keyword');
    expect(__printerTestUtils.printNode(keywordNode, pascalOptions)).toBe('Function');
  });
  ...
  it('printText rewrites generic aliases when enabled', () => {
    const aliasOptions = resolveOptions(createOptions({ powershellRewriteAliases: true }));
    const aliasNode = makeTextNode('write', 'word');
    expect(__printerTestUtils.printNode(aliasNode, aliasOptions)).toBe('Write-Output');
  });

  it('printStatementList applies pending blank lines and indentation', () => {
    const pipeline: PipelineNode = {
      type: 'Pipeline',
      segments: [makeExpressionNode([makeTextNode('Write-Host', 'word')])],
      loc: { start: 0, end: 0 },
    };
    const blankLine: ScriptBodyNode = { type: 'BlankLine', count: 1, loc: { start: 0, end: 0 } };
    const doc = __printerTestUtils.printStatementList([blankLine, pipeline], resolvedOptions, true);
...
function Beta {}`,
    ...
    const doc = __printerTestUtils.printStatementList(script.body, resolvedOptions, false);
    expect(doc).toBeDefined();
  });
...
function Delta {}`,
  ...
  it('printStatementList honours pending blank lines between entries', () => {
    ...
    const blank: ScriptBodyNode = { type: 'BlankLine', count: 2, loc: { start: 0, end: 0 } };
    const doc = __printerTestUtils.printStatementList(
      [firstPipeline, blank, secondPipeline],
      resolvedOptions,
      false,
    );
    expect(doc).toBeDefined();
  ...
  it('powerShellPrinter returns empty doc for undefined nodes', () => {
    const path = { getValue: () => undefined } as unknown as AstPath<ScriptNode>;
    const optionsForPrint = createOptions() as ParserOptions<ScriptNode>;
    const printed = powerShellPrinter.print(path, optionsForPrint, () => '');
    expect(printed).toBe('');
  });
  ...
  it('printNode routes to specialized handlers', () => {
    ...
    const expressionNode: ExpressionNode = makeExpressionNode([makeTextNode('value', 'word')]);
    expressionNode.loc = loc;
    expect(__printerTestUtils.printNode(expressionNode, resolvedOptions)).not.toBe('');
    ...
    expect(__printerTestUtils.printNode(arrayNode, resolvedOptions)).not.toBe('');

    const emptyHashtable: HashtableNode = { type: 'Hashtable', entries: [], loc };
    expect(__printerTestUtils.printNode(emptyHashtable, resolvedOptions)).toBe('@{}');
    ...
    expect(__printerTestUtils.printNode(entryNode, resolvedOptions)).not.toBe('');

    const skippedPartsExpression: ExpressionNode = {
      type: 'Expression',
      parts: [makeTextNode('   `   ', 'operator'), makeTextNode('value', 'word')],
      loc,
    };
    expect(__printerTestUtils.printNode(skippedPartsExpression, resolvedOptions)).not.toBe('');
  });
});
...
describe('parser internal helpers', () => {
  ...
  it('identifies opening and closing tokens', () => {
    ...
    expect(parserUtils.isOpeningToken(tokens[tokens.length - 1])).toBe(false);
  });
  ...
  it('splits hashtable entries across semicolons and newlines', () => {
    ...
    expect(entries[1]?.some((token) => token.value === 'nested')).toBe(true);
  });
  ...
  it('splitArrayElements ignores separators inside nested parentheses', () => {
    ...
    const { contentTokens } = parserUtils.collectStructureTokens(arrayTokens, 0);
  ...
  it('buildExpressionFromTokens classifies array kinds and script blocks', () => {
    const implicitExpression = parserUtils.buildExpressionFromTokens(tokenize('@(1,2)'));
    const implicitArray = implicitExpression.parts.find(
      (part): part is ArrayLiteralNode => part.type === 'ArrayLiteral',
    );
    expect(implicitArray?.kind).toBe('implicit');

    const explicitExpression = parserUtils.buildExpressionFromTokens(tokenize('[1,2]'));
    const explicitArray = explicitExpression.parts.find(
      (part): part is ArrayLiteralNode => part.type === 'ArrayLiteral',
    );
    expect(explicitArray?.kind).toBe('explicit');

    const scriptExpression = parserUtils.buildExpressionFromTokens(tokenize('{ Write-Host }'));
    expect(scriptExpression.parts.some((part) => part.type === 'ScriptBlock')).toBe(true);
  });
  ...
  it('createTextNode maps token roles comprehensively', () => {
    const tokens: Token[] = [
      ...
      { type: 'string', value: '"text"', start: 0, end: 6, quote: 'double' },
      { type: 'operator', value: '+', start: 0, end: 1 },
      { type: 'punctuation', value: ',', start: 0, end: 1 },
      { type: 'unknown', value: '§', start: 0, end: 1 },
    ];

    const roles = tokens.map((token) => parserUtils.createTextNode(token).role);
  ...
  it('parseArrayPart falls back to appropriate end positions when unterminated', () => {
    ...
    const withContentExpression = parserUtils.buildExpressionFromTokens(withContentTokens);
    const withContentArray = withContentExpression.parts.find(
      (part): part is ArrayLiteralNode => part.type === 'ArrayLiteral',
    );
    expect(withContentArray?.loc.end).toBe(withContentTokens[withContentTokens.length - 1]?.end);
  ...
  it('parseScriptBlockPart handles missing closing braces gracefully', () => {
    ...
    const blockExpression = parserUtils.buildExpressionFromTokens(blockTokens);
    ...
    const emptyExpression = parserUtils.buildExpressionFromTokens(emptyTokens);
  ...
  it('resolveStructureEnd respects closing, content, and fallback positions', () => {
    const startToken: Token = { type: 'punctuation', value: '(', start: 0, end: 1 };
    const closingToken: Token = { type: 'punctuation', value: ')', start: 10, end: 11 };
    expect(parserUtils.resolveStructureEnd(startToken, closingToken, [])).toBe(11);
    const contentToken: Token = { type: 'number', value: '1', start: 2, end: 3 };
    expect(parserUtils.resolveStructureEnd(startToken, undefined, [contentToken])).toBe(3);
    expect(parserUtils.resolveStructureEnd(startToken, undefined, [])).toBe(1);
  });
  ...
  it('splitHashtableEntries pops nested closing tokens safely', () => {
    ...
    expect(entries[0]?.filter((token) => token.value === '}')).toHaveLength(2);
  });
