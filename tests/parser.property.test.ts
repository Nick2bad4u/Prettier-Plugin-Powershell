...
import { assertPowerShellParses, isPowerShellParsable } from "./utils/powershell.js";
            ...
            async (tracker) => {
                await fc.assert(
                    fc.asyncProperty(
                        ...
                        async (script) => {
                            tracker.advance();
                            const isValidPowerShell = isPowerShellParsable(
                                script,
                            ...
                            const hasTryCatch =
                                /\btry\b/i.test(script) &&
                                /\bcatch\b/i.test(script);
                            if (!hasTryCatch) {
