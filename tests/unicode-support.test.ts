...
describe("Unicode support in tokenizer", () => {
    it("supports Unicode variable names (Greek)", async () => {
        const script = "$π = 3.14";
        const result = await formatAndAssert(script, baseConfig, { id: "unicode-support.test.ts.result", skipParse: true });
    ...
    it("supports Unicode identifiers (function names)", async () => {
        ...
        const result = await formatAndAssert(script, baseConfig, "unicode-support");
